...
function HandleData() {
    ...
    const [datasub, setDatasub] = useState({ id_cust: 0 });
    ...
    const getSub = async () => {
        let userData = getJWTData();
        const response = await axios.get(
            `http://localhost:3030/getsubmission?id_cust=${userData.data.id_user}`
        );

        let result = response.data.result;
        setDatasub(response.data.result);
        if (
            result &&
            Object.keys(result).length === 0 &&
            Object.getPrototypeOf(result) === Object.prototype
    ...
    return (
        <>
            ...
            <DataPengajuan data={data} />
            <DataKelengkapan datasub={datasub} />
        </>
    );
}

...
function HandleData() {
   ...
   const getIdentity = async () => {
      ...
      const response = await axios.get(`http://localhost:3030/getidentity?id_cust=${userData.data.id_user}`);
   ...
   return (
      <>
         <Header className="site-layout-sub-header-background header_class" style={{ padding: 0, background: "#fff" }}>
            <div className="text_header">Data Pengajuan KPR</div>
         </Header>
         {empty ? <p>Anda belum mengajukan sesuatu.</p> : dataType === FormKPRType.Pengajuan ? <DataPengajuan data={data} /> : <DataKelengkapan data={data} />}
      </>
   );
}
